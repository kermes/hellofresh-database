...
if ($flyout) {
    ...
    $classes = Flux::classes()
        ->add(match ($variant) {
            default => '[:where(&)]:max-w-xl [:where(&)]:min-w-xs shadow-lg rounded-xl',
            'bare' => '',
        })
        ->add(match ($variant) {
...
$contentClass = $styleAttributes->get('class', '') ?: '';
@endphp
...
<ui-modal {{ $attributes }} data-flux-modal>
    ...
    >
        ...
        <?php else: ?>
            <div class="rounded-xl overflow-hidden max-h-[90dvh]">
                <div class="p-6 overflow-x-hidden overflow-y-auto max-h-[90dvh] {{ $contentClass }}">
                    {{ $slot }}
                </div>
            </div>
        <?php endif; ?>
